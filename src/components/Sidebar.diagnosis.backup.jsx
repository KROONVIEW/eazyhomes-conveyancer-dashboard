...
const Sidebar = ({ isCollapsed }) => {
  ...
  const [isDocsSticky, setIsDocsSticky] = useState(false);
    ...
    <div className="flex flex-col h-full w-full">
      ...
      <div className="flex-1 flex flex-col overflow-y-auto overflow-x-hidden p-2 space-y-1">
        ...
        <div
          key="Documents"
          ref={docsDropdownRef}
          onMouseEnter={() => { if (!isDocsSticky && !isCollapsed) setIsDocsOpen(true); }}
          onMouseLeave={() => { if (!isDocsSticky) setIsDocsOpen(false); }}
          onClick={e => {
            e.stopPropagation();
            if (!isCollapsed) {
              setIsDocsOpen(true);
              setIsDocsSticky(true);
            }
          }}
        ...
        >
          ...
          {isDocsOpen && !isCollapsed && ReactDOM.createPortal(
            ...
            >
              <NavLink to="/documents/upload" className={({ isActive }) => `${nestedItemClass} ${isActive ? 'bg-blue-100 text-blue-700 font-semibold' : ''}`} onClick={e => { e.stopPropagation(); setIsDocsOpen(false); setIsDocsSticky(false); setTimeout(() => navigate('/documents/upload'), 0); }}><span className="text-sm truncate">Upload</span></NavLink>
              <NavLink to="/documents/download" className={({ isActive }) => `${nestedItemClass} ${isActive ? 'bg-blue-100 text-blue-700 font-semibold' : ''}`} onClick={e => { e.stopPropagation(); setIsDocsOpen(false); setIsDocsSticky(false); setTimeout(() => navigate('/documents/download'), 0); }}><span className="text-sm truncate">Download</span></NavLink>
            </div>,
            document.body
          )}
        </div>
