...
class GlobalSearchService {
  ...
  async buildSearchIndex() {
    try {// Get all matters
      ...
      this.lastIndexUpdate = Date.now();} catch (error) { // Error logging removed for production
// Fallback to empty index
  ...
  async search(query, options = {}) {
    ...
    if (this.searchCache.has(searchKey)) {
      ...
      if (Date.now() - cached.timestamp < this.cacheTimeout) {return cached.results;
      }
    }
  ...
  async initialize() {await this.buildSearchIndex();}
}
